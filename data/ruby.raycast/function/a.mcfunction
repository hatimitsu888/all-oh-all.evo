...
# 判定エンティティ位置
    execute rotated as @s run \
        tp @s ^ ^ ^ ~ ~
    execute rotated as @s anchored eyes positioned ^ ^ ^64 run \
        tp @n[tag=ruby.raycast] ~ ~-0.015625 ~

# 視点判定
    execute rotated as @s if predicate ruby.raycast:look_interaction positioned ^ ^ ^64 run \
        return run \
            function ruby.raycast:b

# 細分化
    scoreboard players remove $distance ruby.raycast 64000000
    function ruby.raycast:b
